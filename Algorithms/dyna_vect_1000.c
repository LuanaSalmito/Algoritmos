...
int increase_memory(array_list_int* list) {
  ...
  unsigned int new_capacity = list->capacity + 1000;
...
array_list_int* array_list_create() {
  ...
  new_list->data = (int*)malloc(sizeof(int) * 1000); //Iniciando com 1000
...
double array_list_percent_occupied(array_list_int* list) {
  ...
  double occupied_percent = ((double)list->size / (double)list->capacity) * 1000.0;
